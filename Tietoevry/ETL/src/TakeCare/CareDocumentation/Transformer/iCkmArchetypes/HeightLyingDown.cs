...
{
    internal record HeightLyingDown
    {
        private static IUnitProvider _unitService;
        private static ITerminologyProvider _lookupProvider;
        //static constructor
        static HeightLyingDown()
        {
            _unitService = new UnitProvider();
            _lookupProvider = new TerminologyProvider();
        }

        public static void AddHeightLyingDownData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/längd_liggande");
            ...
            {
                composedObject[$"{prefix}{v}{"/kroppslängd|magnitude"}"] = keyword.Value.NumVal.Val;
                ...
                if (!string.IsNullOrEmpty(keyword.Value.NumVal.Unit))
                {
                    ...
                    composedObject[$"{prefix}{v}{"/kroppslängd|unit"}"] = openEhrUnit;
                ...
                {
                    throw new System.Exception("Height Lying Down : Unit is missing");
                }
            }
            composedObject[$"{prefix}{v}{"/kroppsställning|code"}"] = "at0020";
            composedObject[$"{prefix}{v}{"/kroppsställning|value"}"] = "Liggande";
            composedObject[$"{prefix}{v}{"/kroppsställning|terminology"}"] = "local";
