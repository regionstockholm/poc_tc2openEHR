...
{
    internal record News2TotalScoreHeartRate
    {
        private readonly IUnitProvider _unitService;

        public News2TotalScoreHeartRate(IUnitProvider unitService)
        {
            _unitService = unitService;
        }

        public static void AddTotalScoreHeartRateData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/news2_totalpoäng_hjärtfrekvens");
