...
{
    internal record DiastolicHoArm
    {

        private static IUnitProvider _unitService;
        private static ITerminologyProvider _lookupProvider;
        //static constructor
        static DiastolicHoArm()
        {
            _unitService = new UnitProvider();
            _lookupProvider = new TerminologyProvider();
        }

        public static void AddDiastolicHoArmData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/blodtryck_hö_arm_diastoliskt");
            ...
            if (keyword.Value != null && keyword.Value.NumVal != null)
            {
                ...
                {
                    throw new System.Exception("DiastolisktHoArm : Unit is missing");
                }
            }
            composedObject[$"{prefix}{v}{"/mätplats|code"}"] = "at0025";
            composedObject[$"{prefix}{v}{"/mätplats|value"}"] = "Höger arm";
