...
namespace TakeCare.Migration.OpenEhr.CareDocumentation.Transformer.Models
{
    internal record BirthWeight
    {
        ...
        public static void AddBirthWeightData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/födelsevikt");
            ...
            if (keyword.Value != null && keyword.Value.NumVal != null)
            {
                if (string.IsNullOrEmpty(keyword.Value.NumVal.Unit))
                {
                    ...
                    {
                        throw new System.Exception("BirthWeight : Unit is missing");
                    }
                }
                else
                {
