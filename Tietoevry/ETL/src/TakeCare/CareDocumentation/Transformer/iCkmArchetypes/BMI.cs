...
namespace TakeCare.Migration.OpenEhr.CareDocumentation.Transformer.Models
{
    internal record BMI
    {
        ...
        public static void AddBMIData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            ...
            {
                composedObject[$"{prefix}{v}{"/kroppsmasseindex|value"}"] = keyword.Value.NumVal.Val;
                composedObject[$"{prefix}{v}{"/kroppsmasseindex|unit"}"] = "kg/m2";
                ...
                if (!string.IsNullOrEmpty(keyword.Value.NumVal.Unit))
                {
                    ...
                    composedObject[$"{prefix}{v}{"/kroppsmasseindex|unit"}"] = openEhrUnit;
