...
{
    internal record MeanArterialPressure
    {
        private static IUnitProvider _unitService;
        private static ITerminologyProvider _lookupProvider;
        //static constructor
        static MeanArterialPressure()
        {
            _unitService = new UnitProvider();
            _lookupProvider = new TerminologyProvider();
        }

        public static void AddMeanArterialPressureData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {   
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/map");
            ...
            {
                composedObject[$"{prefix}{v}{"/medelartärtryck|magnitude"}"] = keyword.Value.NumVal.Val;
                ...
                if (!string.IsNullOrEmpty(keyword.Value.NumVal.Unit))
                {
                    ...
                    composedObject[$"{prefix}{v}{"/medelartärtryck|unit"}"] = openehrUnit;
                ...
                {
                    throw new System.Exception("Mean Arterial pressure : Unit is missing");
                }
            } 
