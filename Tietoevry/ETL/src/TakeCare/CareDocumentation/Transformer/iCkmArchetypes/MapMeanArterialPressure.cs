...
namespace TakeCare.Migration.OpenEhr.CareDocumentation.Transformer.Models
{
    internal record MeanArterialPressure
    {
        ...
        public MeanArterialPressure(IUnitProvider unitService)
        {
            _unitService = unitService;
        }

        public static void AddMeanArterialPressureData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix)
        {   
            ...
            composedObject[$"{prefix}{v}{"/medelartärtryck|magnitude"}"] = (keyword.Value != null) ? ((keyword.Value.NumVal != null) ? keyword.Value.NumVal.Val : keyword.Value.TextVal) : "";
            composedObject[$"{prefix}{v}{"/medelartärtryck|unit"}"] = "mm[Hg]"; 
