...
{
    internal record SystolicHoArm
    {
        private static IUnitProvider _unitService;
        private static ITerminologyProvider _lookupProvider;
        //static constructor
        static SystolicHoArm()
        {
            _unitService = new UnitProvider();
            _lookupProvider = new TerminologyProvider();
        }

        public static void AddSystolicHoArmData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/blodtryck_hö_arm_systoliskt");
            ...
            if (keyword.Value!=null && keyword.Value.NumVal != null)
            {
                ...
                {
                    throw new System.Exception("SystolicHoArm : Unit is missing");
                }
            }
