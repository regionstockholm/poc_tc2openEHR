...
{
    internal record DiastolicInvasive
    {
        private static IUnitProvider _unitService;
        private static ITerminologyProvider _lookupProvider;
        //static constructor
        static DiastolicInvasive()
        {
            _unitService = new UnitProvider();
            _lookupProvider = new TerminologyProvider();
        }

        public static void AddDiastolicData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/invasivt_blodtryck_diastoliskt");
            ...
            if (keyword.Value!=null && keyword.Value.NumVal != null)
            {
                ...
                {
                    throw new System.Exception("DiastolisktInvasive : Unit is missing");
                }
            }
