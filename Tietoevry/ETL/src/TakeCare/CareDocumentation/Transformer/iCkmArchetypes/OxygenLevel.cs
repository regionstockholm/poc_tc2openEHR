...
namespace TakeCare.Migration.OpenEhr.CareDocumentation.Transformer.Models
{
    internal record OxygenLevel
    {
        ...
        public OxygenLevel(IUnitProvider unitService)
        {
            _unitService = unitService;
        }

        public static void AddOxygenLevelData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/syrgasnivå");
            prefixBuilder.Append(":");
            string prefix = prefixBuilder.ToString(); 
            string suffix = "/inandad_syrgas/procent_o/";
            composedObject[$"{prefix}{v}{suffix}{"/_uid"}"] = keyword.Guid;
            composedObject[$"{prefix}{v}{suffix}{"|numerator"}"] = 2.17;
            composedObject[$"{prefix}{v}{suffix}{"|denominator"}"] = 100;
            composedObject[$"{prefix}{v}{suffix}{"|type"}"] = 2;
