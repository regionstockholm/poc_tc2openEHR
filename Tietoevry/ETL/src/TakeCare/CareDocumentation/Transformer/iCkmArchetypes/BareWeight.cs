...
namespace TakeCare.Migration.OpenEhr.CareDocumentation.Transformer.Models
{
    internal record BareWeight
    {
        ...
        public static void AddBareWeightData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/nakenvikt");
            ...
            if (keyword.Value != null && keyword.Value.NumVal != null)
            {
                if (string.IsNullOrEmpty(keyword.Value.NumVal.Unit))
                {
                    if (!string.IsNullOrEmpty(termCatalog.Unit))
                    {
                        ...
                        composedObject[$"{prefix}{v}{"/vikt|magnitude"}"] = keyword.Value.NumVal.Val;
                        composedObject[$"{prefix}{v}{"/vikt|unit"}"] = openEhrUnit;
                    ...
                    {
                        throw new System.Exception("BareWeight : Unit is missing");
                    }
                }
                else
                {
            ...
            composedObject[$"{prefix}{v}{"/klädsel|code"}"] = "at0013";
            composedObject[$"{prefix}{v}{"/klädsel|value"}"] = "Naken";
            composedObject[$"{prefix}{v}{"/klädsel|terminology"}"] = "local";
