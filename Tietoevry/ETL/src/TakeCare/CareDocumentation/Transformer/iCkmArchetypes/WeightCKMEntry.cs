...
namespace TakeCare.Migration.OpenEhr.CareDocumentation.Transformer.Models
{
    internal record WeightCKMEntry
    {
        ...
        //static constructor
        static WeightCKMEntry()
        {
            _unitService = new UnitProvider();
            _lookupProvider = new TerminologyProvider();
        }

        public static void AddWeightData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix, TerminologyDetails termData)
        {
            ...
            if (keyword.Value!=null && keyword.Value.NumVal != null)
            {
                ...
                else if (!string.IsNullOrEmpty(termCatalog.Unit))
                {
                    ...
                    throw new System.Exception("Weight : Unit is missing");
                }
            }
