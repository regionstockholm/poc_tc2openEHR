...
{
    internal record PulseRate
    {
        private readonly IUnitProvider _unitService;

        public PulseRate(IUnitProvider unitService)
        {
            _unitService = unitService;
        }

        public static void AddPulseRateData(JObject composedObject, KeywordDto keyword, int v, string commonPrefix)
        {
            StringBuilder prefixBuilder = new StringBuilder(commonPrefix);
            prefixBuilder.Append("ickm/pulsfrekvens");
            prefixBuilder.Append(":");
            string prefix = prefixBuilder.ToString();
            composedObject[$"{prefix}{v}{"/_uid"}"] = keyword.Guid;
            composedObject[$"{prefix}{v}{"/frekvens|magnitude"}"] = (keyword.Value != null) ? ((keyword.Value.NumVal != null) ? keyword.Value.NumVal.Val : keyword.Value.TextVal) : "";
            composedObject[$"{prefix}{v}{"/frekvens|unit"}"] = (keyword.Value != null) ? ((keyword.Value.NumVal != null) ? keyword.Value.NumVal.Unit : "") : "";
